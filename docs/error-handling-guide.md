# Erros! Sobre controle

## Indíce

1. Erros de Interface (UI)
2. Erros de Infraestrutura/Rede.
3. Configurando ambiente de erro.
4. Erros de Negócio.

### 1. Erros de Interface (UI)

No começo do projeto já havia pensado em como centralizar os erros, pois tive experiência
semelhantes no desenvolvimento beckend, como a de tratar os erros e passar para o usuário mensagens
amigavéis, por isso antes mesmo da implementação de algumas funcionalidades e avançar mais afundo no
...
[logleverl](https://www.npmjs.com/package/loglevel?activeTab=readme#developing--contributing), onde
encotrei motivos densos para configurar meu interceptador axio, para tratar erros request e
response.

### 3. Configurando ambiente de erro.

Foi preciso aprofundar em pesquisa, como à de como limitar os erros que quebram minha aplicação
durante o uso, para isso usei o **ErrorBoundary**, verifiquei na documentação react como usar esse
componente, bem como posso complementar a camada de `perfomace`, (veja em
[performace](./docs/performace.md)), alinhada com a lib useQuery.
...
- **loglevel**: Usada para tratamento de erros em ambiente de desenvolvimento, evitando uso de
  console log.
...
- **@tanstack/react-query**: Melhorar a performace da aplicação, delegando os estados dos
  componentes visuais de modo que tratamento de erros e loading receba status, podendo manter e
  acessar o status caches.
